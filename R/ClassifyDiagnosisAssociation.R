...
#-------------------------------------------------------------------------------
  ...
  Res.ICD10Group <- dsCCPhos::Res.CancerGrouping %>%
                          select(ICD10Code.Short,
                                 CancerTopographyGroup.ICD10) %>%
                          rename(ICD10Group = CancerTopographyGroup.ICD10)


  # Enhance diagnosis entries by auxiliary variables
  DiagnosisEntries <- DiagnosisEntries %>%
                          arrange(DiagnosisDate, HistologyDate) %>%
                          mutate(ICD10Code.Short = case_when(str_starts(ICD10Code, "C") ~ str_sub(ICD10Code, end = 3),
                                                            str_starts(ICD10Code, "D") ~ str_sub(ICD10Code, end = 5),
                                                            TRUE ~ str_sub(ICD10Code, end = 3)),
                                 ICDOTopographyCode.Short = str_sub(ICDOTopographyCode, end = 3),
                                 ICDOMorphologyCode.Short = str_sub(ICDOMorphologyCode, end = 4)) %>%
                          left_join(Res.ICD10Group, by = join_by(ICD10Code.Short))
  ...
  while (nrow(Candidates) > 0)
  {
      ...
      Associations <- Candidates %>%
                          ...
                          mutate(ReferenceDiagnosisID = Reference$DiagnosisID,
                                 ...
                                 Relation.ICD10 = eval(parse(text = RuleCalls[["Relation.ICD10"]])),
                                 Relation.ICDOTopography = eval(parse(text = RuleCalls[["Relation.ICDOTopography"]])),
                                 Relation.LocalizationSide = eval(parse(text = RuleCalls[["Relation.LocalizationSide"]])),
                                 Relation.ICDOMorphology = eval(parse(text = RuleCalls[["Relation.ICDOMorphology"]])),
                                 Relation.Grading = eval(parse(text = RuleCalls[["Relation.Grading"]])),
  ...
  Output <- Output %>%
                select(-c(ICD10Code.Short,
                          ICD10Group,
                          ICDOTopographyCode.Short,
                          ICDOMorphologyCode.Short)) %>%
